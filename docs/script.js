            ...
            <div class="flex flex-col md:flex-row">
                <div class="md:w-1/2 relative overflow-hidden">
                    <div class="aspect-video relative">
                        <img src="${project.image || "assets/placeholder.jpg"}" alt="${project.title}" class="w-full h-full object-cover" style="image-rendering: pixelated;">
                ...
                <div class="md:w-1/2 p-6 md:p-8">
                    ...
                    <button class="pixel-button bg-green-600 hover:bg-green-500 text-white font-pixel py-2 px-6 rounded-sm flex items-center view-details-button" data-index="${index}">
                        VER DETALHES
                    </button>
                </div>
            </div>
  ...
  modalContent.innerHTML = `
        <div class="p-6">
            ...
            <div class="modal-image-container">
                <img src="${project.image || "assets/placeholder.jpg"}" alt="${project.title}" class="modal-image">
            </div>
            ...
            <div class="modal-content space-y-4">
                ...
                <div class="modal-play-button-container">
                    <a href="${project.link}" target="_blank" rel="noopener noreferrer" class="modal-play-button">
                        JOGAR AGORA <span class="gamepad-icon ml-3"></span>
                    </a>
                </div>
                
                <p class="modal-redirect-note">
                    <span class="external-link-icon inline-block w-4 h-4 mr-1"></span>
...
function createBackgroundParticles() {
  ...
  gridPattern.innerHTML = `
        <div class="w-full h-full" style="background-image: linear-gradient(to right, #10B981 1px, transparent 1px), linear-gradient(to bottom, #10B981 1px, transparent 1px); background-size: 20px 20px;"></div>
    `
  backgroundContainer.appendChild(gridPattern)

  // Add finance symbols
  const symbols = ["$", "€", "£", "¥", "%"]
  const colors = ["#10B981", "#3B82F6", "#F59E0B", "#6366F1", "#047857"]
  ...
  for (let i = 0; i < 15; i++) {
    ...
    symbol.className = "absolute font-bold pixel-float"
