            ...
            <div class="modal-content space-y-4">
                ...
                <div class="modal-play-button-container">
                    <a href="${project.link}" target="_blank" rel="noopener noreferrer" class="modal-play-button">
                        JOGAR AGORA <img src="assets/icons/gamepad.png" alt="Gamepad" class="w-6 h-6 ml-3">
                    </a>
                </div>
                
                <p class="modal-redirect-note">
                    <img src="assets/icons/external-link.png" alt="External Link" class="inline-block w-4 h-4 mr-1">
...
function createBackgroundParticles() {
  ...
  // Add grid pattern - Usando azul em vez de verde
  const gridPattern = document.createElement("div")
  gridPattern.className = "absolute inset-0 opacity-5"
  gridPattern.innerHTML = `
        <div class="w-full h-full" style="background-image: linear-gradient(to right, #3B82F6 1px, transparent 1px), linear-gradient(to bottom, #3B82F6 1px, transparent 1px); background-size: 20px 20px;"></div>
    `
  backgroundContainer.appendChild(gridPattern)

  // Add finance symbols
  const symbols = ["$", "€", "£", "¥", "%"]
  const colors = ["#3B82F6", "#60A5FA", "#F59E0B", "#6366F1", "#93C5FD"]
